...
import model._
import Main._
import model.newsComponents.blockNewsComponents.{NewsBlockPrimeModel, NewsBlockPrimeTable}
...
object NewsBlockPrimeRepo {
  ...
  def updateData(updatedData: NewsBlockPrimeModel): Future[Int] = {
    val updateAction = NewsBlockPrimeQuery.filter(_.id === updatedData.id)
      .map(newsBlock => (newsBlock.authorId, newsBlock.canComment, newsBlock.authorCategory, newsBlock.date, newsBlock.filter, newsBlock.importance, newsBlock.lifetime, newsBlock.titel, newsBlock.like, newsBlock.dislike))
      .update((updatedData.authorId, updatedData.canComment, updatedData.authorCategory, updatedData.date, updatedData.filter, updatedData.importance, updatedData.lifetime, updatedData.titel, updatedData.like, updatedData.dislike))
